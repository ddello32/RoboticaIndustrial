...
%% Simulacao 4
q0 = [0 pi/2+0.05 -pi/2 0 0 0]; %Posicao inicial
[time, qdyn4, qddyn] = bot.fdyn(5, T, q0, zeros(1,6));
sim4ode = figure;
plot(time, qdyn4);
xlabel('Tempo(s)')
ylabel('Angulo(rad)')
...
[time, qdyn, qddyn] = p560.nofriction('all').fdyn(30, T, q0, zeros(1,6));
sime2ode = figure;
plot(time, qdyn);
xlabel('Tempo(s)')
ylabel('Angulo(rad)')
...
title('Resposta de q')
legend('q_1', 'q_2', 'q_3', 'q_4', 'q_5', 'q_6');
saveas(sime3odea,'sime3odea.eps','epsc')
sime3kina = figure;
m = bot.inertia(qdyn);
e = zeros(1, length(qdyn));
for i = 1:length(qdyn)
...
ylabel('Energia(J)')
title('Energia cinetica do sistema')
saveas(sime3kina,'sime3kina.eps','epsc')
figure,
bot.plot(qdyn, 'delay', 5/length(time))
